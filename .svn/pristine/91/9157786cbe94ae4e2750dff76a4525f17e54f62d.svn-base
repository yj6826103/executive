<template>
  <div class="box table-box">
    <el-table class="table table-border table-bg" :data="acTeamInfoList" stripe @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="statusd" label="备注" ></el-table-column>
    ...
    <div class="btnsbox" v-show="acTeamInfoList.length>0">
      ...
      _this.$http
        ...
        .then(res => {
          ...
          if (res.data.status == '100') {
            for(let i of res.data.data.list){//console.log(i.status)
              if(i.status=='5'){
                i.statusd='删除'
              }else if(i.status=='6'){
                i.statusd='修改'
