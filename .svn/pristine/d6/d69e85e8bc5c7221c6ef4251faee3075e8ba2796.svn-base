...
export default {
  ...
  methods: {
        ...
        .then(response => {//console.log(response.data)
          if (response.data.status == 100) {          
            this.days.forEach((item,j)=>{
                let sdata=this.formatDate(item.FullYear,item.Month,item.date)
                response.data.data.forEach((items, index) => {
                    let stndStart=items.stndStart.substring(0,10)
                    if(sdata==stndStart){
                        this.days[j].num=items.num;
                    }
                })         
            })
          }
    ...
    initData(cur) {
        let date,getdd,lastDay
        if (cur) {
            date = new Date(cur);
            let myDate = new Date(date.getFullYear(),date.getMonth(), 0);
        ...
        let strd = this.formatDate(
            ...
            this.currentMonth-1==0?12:this.currentMonth-1,
            this.currentDay
        );
        for (var i = 0; i <=getdd; i++) {//上个月后几天
            let DurMonth={}
            let d = new Date(strd);
        ...
        for (var i = this.currentDay-1; i >= 0; i--) {//当前月当前日期之前
            let DurMonth={}
            let d = new Date(str);
