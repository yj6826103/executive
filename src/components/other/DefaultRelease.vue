  ...
  <div class="defaultRelease">
        <div class="popbox-cen dd-box">
            <div class="rbox">
                ...
                <div class="info-box2">
                    <div class="rbbq">
                        ...
                        <div class="cen cen21">
                            <table>
                                <tr>
                                    <th width="70%" class="filename" v-if="bizOrder.bocDoc!=null" v-text="bizOrder.bocDoc.docName"></th>
                                    <th width="30%">
                                        <singleFileUpload v-if="bizOrder.bocDoc!=null"
                                            :fileType="5" 
                                            :disabled="orderBocStatus==2" 
                                            :fileName="bizOrder.bocDoc.docName" 
                                            :serverFileName="bizOrder.bocDoc.docAddress" 
                ...
                <div class="btnbox info-box3" v-if="bizOrder.orderBocStatus==1">
...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      let _this = this
      _this.$http.post('/bizOrder/selectBreakNotify', params, {
          headers: { Authorization: localStorage.getItem('token') }
        })
      .then(res=> {//console.log(res)
        if (res.data.status == '100') {
          _this.bizOrder = res.data.data.list[0]
          // this.fileName=_this.bizOrder.bocDoc.docName
    ...
    submit() {
      let _this = this
      _this.$http.post('/bizOrder/orderToNotify',qs.stringify(_this.bizOrder, _this.QS_STRINGIFY_OPTIONS),
          { headers: { Authorization: localStorage.getItem('token') } 
      })
      .then(res=> {
        const h = _this.$createElement
