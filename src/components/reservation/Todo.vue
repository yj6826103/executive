  ...
  <div class="todo">
      <div class="yuding-cen">
          <el-table class="table table-border table-bg" :data="datas" stripe :row-class-name="tableRowClassName">
                ...
                <el-table-column prop="date1" :label="$t('Scheduleddate')"></el-table-column><!--预订日期-->
                <el-table-column prop="orderDArptCode" :label="$t('msg.Cityfrom')" ></el-table-column><!--出发地-->
                <el-table-column prop="sta" :label="$t('msg.ScheduledArrival')"  min-width='90px'></el-table-column><!--计划进港日期-->
                <el-table-column prop="orderArptName" :label="$t('ParkingAirport')" ></el-table-column><!--停靠机场-->
                <el-table-column prop="std" :label="$t('msg.ScheduledDeparture')"  min-width='90px'></el-table-column><!--计划出港日期-->
                ...
                <el-table-column :label="$t('Orderstatus')" ><!--订单状态-->
                    <template slot-scope="scope">
                        <label v-if="scope.row.orderStatus==4">{{$t('Dismissal')}}</label><!--已驳回-->
                        <label v-if="scope.row.orderStatus==1">{{$t('Saved')}}</label><!--已保存-->
                    </template>
                </el-table-column>
                <el-table-column :label="$t('DismissalReason')" ><!--驳回原因-->
                    <template slot-scope="scope">
                        <el-tooltip class="item" effect="light" placement="top">
                            <div slot="content" v-text="scope.row.orderReturnedReason"></div>
                            <span class="span_tooltip">{{scope.row.orderReturnedReason}}</span>
                        </el-tooltip>
                    </template>
                </el-table-column>
                ...
                <el-table-column :label="$t('operation')"  min-width='110px'><!--操作-->
                    <template slot-scope="scope">
                        <button class="button primary" @click="modOrder(scope.row)">{{$t('Edit')}}</button>
                        <button class="button success" style="background: #67c23a !important;" @click="commitData(scope.row.orderId,scope.row.orderNo,scope.row.orderType,scope.row.orderStd,scope.$index)">{{$t('Submit')}}</button>
                        <i class="el-icon-circle-close-outline" v-if="scope.row.orderStatus==1" @click="deleteOrder(scope.row.orderId,scope.row.orderNo,scope.$index)"></i>
...
export default {
    ...
    methods: {
        ...
        commitData(orderId,orderNo,orderType,orderStd,index){
            this.$confirm(this.$t('is')+this.$t('Submit')+'?', this.$t('Prompt'), {
                confirmButtonText: this.$t('Confirm'),
                cancelButtonText: this.$t('cancel'),
            }).then(() => {
                this.update1(orderId,orderNo,orderType,orderStd,index);           
            }).catch(()=>{})
       },
        update1(orderId,orderNo,orderType,orderStd,index){//提交订单
            let param={
                orderNo:orderNo,
                orderType:orderType,
                orderStd:orderStd,
