<template>
		...
		<div class="createOrderBox">
          ...
          <div class="validateStyle"><span v-show="errors.has('orderNo')" class="help is-danger">{{ errors.first('orderNo') }}</span></div>													
              ...
              <div class="validateStyle"></div>
						...
						<li style="flex:2.05">
							<div class="rbbq">
								<div class="tit"><i class="el-icon-caret-right"></i>{{$t('Causeofdelay')}}</div>
								<div class="cen">
              ...
              <div class="bottom_time">
                ...
                <div class="rbbqOther">
                  <div class="rbbq" style="margin-left:10px">
                    <div class="tit"><i class="el-icon-caret-right"></i>{{$t('msg.Departure')}}{{$t('msg.Flight')}}</div>
                    <div class="cen">
                      <label>
                          <input v-validate data-vv-rules="required" data-vv-as="航班号" name="orderDFlgtNo" type="text" v-model="ChooseOrder.orderDFlgtNo" :placeholder="$t('pleaseenter')" >
                      </label>
                    </div>
                  </div>
              ...
              <input type="text" v-model="orderStay" v-show="false" v-validate data-vv-rules="required" name="orderStaySTATUS">
              ...
              <div class="validateStyle"></div>
							<div class="rbbq">
                <div class="tit gray"><i class="el-icon-caret-right gray"></i>{{$t('Stand')}}</div><!--机位-->
								<div class="cen">
              ...
              <div class="validateStyle"></div>
						...
						<li>
							<div class="rbbq" :class="errClassed?errClass:''">
								...
								<div class="cen">
									<input class="inputText" disabled style="background:none;width:100%" type="text" v-model="orderStay" readonly  placeholder="————" />
								</div>
							</div> 
              <div class="validateStyle"></div>
              <div class="rbbq">
                  <div class="tit gray"><i class="el-icon-caret-right gray"></i>{{$t('Delaytimes')}}</div><!--延期次数-->
                  <div class="cen">
                    <label>
              ...
              <div class="validateStyle"><span v-if="fileFALSE">{{$t('Required')}}</span></div>	
						</div>
						<div class="tab-cell right">
              <div  style="display:flex">
                <div class="rbbq" style='flex:1;margin-right:10px'>
                  <div class="tit"><i class="el-icon-caret-right"></i>{{$t('msg.Airport')}}</div>			    
                  <div class="cen cen21">
                    <el-select  :value="orderArptCode" @change="changeArptCode" placeholder="———————">
                      <el-option
    ...
    <el-dialog :title="$t('Selectorder')"
        ...
        <Bounced ref="Bounceds" v-if="selectFalse" :message="status" @setSelectFalse='set_selectFalse' @initHangar="initHangars"></Bounced>
    </el-dialog>
    <!-- 余量查询 -->
    <el-dialog 
...
export default {
  ...
  methods: {
    ...
    createFilter(queryString) {
      return restaurant => {
        return (
          restaurant.dicCh.toLowerCase().indexOf(queryString.toLowerCase()) ===0
        );
      };
    },
    initHangars() {
      ...
      this.$refs.clearUpFile.closeFile()//清空附件
    ...
    saveSubmit(index) {
      this.$validator.validateAll().then(result => {
       if( this.orderIsappliedHangar==2&&this.ChooseOrder.orderPostponeNum>=3){
        this.scrollToped();
        this.fileFALSE=true;
        this.$notify.warning({
          title: this.$t('Prompt'),
          message: "未申请机库,延期不可超三次！",
          offset: 100
        });
         return;
       }
