<template><!--修改订单-->
    <div class="modMaintainInfo">
        <div v-for="(item,index) in list">
            <div class="mod_box" >
                <div class="mod_list">
                    <label>{{$t('msg.AircraftregistrationNO')}}：</label>
                ...
                <div class="mod_list">
                    <label>{{$t('msg.Aircrafttype')}}：</label>
                ...
                <div class="mod_list">
                    <label style="padding-right:10px">{{$t('company.Owned')}}/{{$t('company.Entrusted')}}：</label>
                    <div class="mod_border mod_list_radio">
                        <el-radio class="mod_list_radio_label" v-model="item.actOwnType" label="所属">{{$t('company.Owned')}}</el-radio>
                        <el-radio class="mod_list_radio_label" v-model="item.actOwnType" label="委托">{{$t('company.Entrusted')}}</el-radio>
                    </div>
                </div>
                
                <div class="mod_list">
                    <label>{{$t('company.AircraftOwner')}}:</label>
                    <div class="mod_border">
                        <input type="text" class="input-text radius" v-model="item.actOwner" :placeholder="$t('pleaseenter')" v-if="item.actOwnType=='委托'">
                        <input type="text" class="input-text radius" v-model="item.actOwner" :placeholder="$t('pleaseenter')" v-else disabled style="background:#fff;color:#ccc">
                    </div>
                </div>
                                                        
                <div class="mod_list">
                    <label>{{$t('company.Aircraft')}}：</label>
                    <div class="mod_border">
                        <singleFileUpload
                            :fileType="fileType"
                            :index="index"
