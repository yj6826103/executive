  ...
  <div class="addMaintainInfo">
      ...
      <table class="table table-border table-bg">
            ...
            <tr v-for="(actTeam,index) in list">
                ...
                <td>
                    <input type="text" class="input-text radius" v-model="actTeam.actOwner" :placeholder="$t('pleaseenter')" disabled v-if="actTeam.actOwnType=='所属'" style="background:#fff;color:#999">
                    <input type="text" class="input-text radius" v-model="actTeam.actOwner" :placeholder="$t('pleaseenter')"  v-else style="background:#fff;color:#666">
                </td>
                <td class="text-right">
                    <singleFileUpload
                        :fileType="fileType" 
  ...
  methods: {
    ...
    submit() {
      if(this.list.length==0){
        this.$notify({
          title: this.$t('Prompt'),
          message:  this.$t('Thesubmissionotempty'),//'提交数据不可为空',
          type: 'warning'
        })
        return
      }
      ...
      _this.$validator.validateAll().then(result => {
        if (result) {
          if(this.isRepeat(this.list)==true){
            this.$notify({
              title: this.$t('Prompt'),
              message:  this.$t('theaircraftisunique'),//'飞机注册号具有唯一性，请重新填写！',
              type: 'warning'
            })
            return
          }
